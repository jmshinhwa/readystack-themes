...
const S = {
  title: 'Python Upgrade Lint: Removed Stdlib 3.12-3.14',
  need_key: 'Full version: scan the whole workspace in one pass and export a Markdown upgrade report per target Python version. $29 once · one licence key per person or team seat. By hand it is developer hours at a $63.98 US median hourly wage (BLS, May 2024).',
...
async function checkWorkspace(ctx) {
  ...
  if (!total) { const _m = 'Swept ' + uris.length + ' files — ' + S.clean; try { if (require('./auto.js').sweptClean(vscode, ctx, { msg: _m, title: S.title, slug: 'python-upgrade-removed-api-lint', prefix: PREFIX })) return; } catch (e) {} vscode.window.showInformationMessage(_m); return; }   // s163 — the clean sweep offers the README badge (auto.js)
  if (!hasKey && !inTrial) {
...
function activate(ctx) {
  ...
  try { lic.pullFeed(ctx, 'python-upgrade-removed-api-lint').then(function (f) { if (f && Array.isArray(f.rules) && ENGINE && Array.isArray(ENGINE.RULES)) { globalThis.__yjFeed = f; for (var i = 0; i < f.rules.length; i++) ENGINE.RULES.push(f.rules[i]); } }).catch(function () {}); } catch (e) {}
  ...
  try { require('./auto.js').start(ctx, { vscode: vscode, ENGINE: ENGINE, GLOB: GLOB, PREFIX: PREFIX, title: S.title, slug: 'python-upgrade-removed-api-lint', price: 29 }); } catch (e) {}
}
