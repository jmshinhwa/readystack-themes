...
const S = {
  title: 'CSV Kassenpruefer - DSFinV-K Export Lint',
  need_key: 'Vollversion: der komplette Exportordner in einem Lauf — alle 20 DSFinV-K-Tabellen zusammen, inklusive der Kreuzprüfungen zwischen den Dateien und der index.xml — und der Prüfbericht als Datei. $29 einmalig · ein Lizenzschlüssel pro Person oder Team-Sitzplatz · 7 Tage volle Rückerstattung. Eine Steuerberatungskanzlei rechnet die Durchsicht nach § 13 StBVV mit 16,50 bis 41 Euro je angefangene Viertelstunde ab.',
  enter_key: 'Lizenzschlüssel eingeben', buy: 'Vollversion holen — $29',
  key_ok: 'Lizenzschlüssel angenommen. Danke.', key_bad: 'Dieser Schlüssel wurde nicht bestätigt. Bitte auf Tippfehler prüfen oder die Vollversion holen.',
  clean: 'Clean against all ' + (ENGINE.RULE_COUNT || (ENGINE.RULES || []).length) + ' checks.'
};
const PREFIX = 'dsfinvkLint';
const GLOB = '**/*.csv';
...
async function checkWorkspace(ctx) {
  ...
  if (!total) { const _m = 'Swept ' + uris.length + ' files — ' + S.clean; try { if (require('./auto.js').sweptClean(vscode, ctx, { msg: _m, title: S.title, slug: 'dsfinv-k-export-lint', prefix: PREFIX })) return; } catch (e) {} vscode.window.showInformationMessage(_m); return; }   // s163 — the clean sweep offers the README badge (auto.js)
  if (!hasKey && !inTrial) {
...
function activate(ctx) {
  ...
  try { lic.pullFeed(ctx, 'dsfinv-k-export-lint').then(function (f) { if (f && Array.isArray(f.rules) && ENGINE && Array.isArray(ENGINE.RULES)) { globalThis.__yjFeed = f; for (var i = 0; i < f.rules.length; i++) ENGINE.RULES.push(f.rules[i]); } }).catch(function () {}); } catch (e) {}
  ...
  try { require('./auto.js').start(ctx, { vscode: vscode, ENGINE: ENGINE, GLOB: GLOB, PREFIX: PREFIX, title: S.title, slug: 'dsfinv-k-export-lint', price: 29 }); } catch (e) {}
}
