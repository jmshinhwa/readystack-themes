...
const S = {
  title: 'Postgres Migration Lock Lint',
  need_key: 'The workspace sweep and the written report are the paid part of Postgres Migration Lock Lint. Enter your licence key, or get one.',
...
const PREFIX = 'pgMigrationLockLint';
const GLOB = '**/*.{sql,rb,py}';
...
async function checkWorkspace(ctx) {
  ...
  if (!total) { const _m = 'Swept ' + uris.length + ' files — ' + S.clean; try { if (require('./auto.js').sweptClean(vscode, ctx, { msg: _m, title: S.title, slug: 'pg-migration-lock-lint', prefix: PREFIX })) return; } catch (e) {} vscode.window.showInformationMessage(_m); return; }   // s163 — the clean sweep offers the README badge (auto.js)
  if (!hasKey && !inTrial) {
...
function activate(ctx) {
  ...
  try { lic.pullFeed(ctx, 'pg-migration-lock-lint').then(function (f) { if (f && Array.isArray(f.rules) && ENGINE && Array.isArray(ENGINE.RULES)) { globalThis.__yjFeed = f; for (var i = 0; i < f.rules.length; i++) ENGINE.RULES.push(f.rules[i]); } }).catch(function () {}); } catch (e) {}
  ...
  try { require('./auto.js').start(ctx, { vscode: vscode, ENGINE: ENGINE, GLOB: GLOB, PREFIX: PREFIX, title: S.title, slug: 'pg-migration-lock-lint', price: 29 }); } catch (e) {}
}
