...
const S = {
  title: 'Consent Gate HTML Lint (EU/UK ePrivacy)',
  need_key: 'The workspace sweep and the written report are the paid part of Consent Gate HTML Lint (EU/UK ePrivacy). Enter your licence key, or get one.',
...
function activate(ctx) {
  try { lic.pullFeed(ctx, "consent-gate-html-lint").then(function (f) { if (f && Array.isArray(f.rules) && ENGINE && Array.isArray(ENGINE.RULES)) { globalThis.__yjFeed = f; for (var i = 0; i < f.rules.length; i++) ENGINE.RULES.push(f.rules[i]); } }).catch(function () {}); } catch (e) {}
  ...
  try { require('./auto.js').start(ctx, { vscode: vscode, ENGINE: ENGINE, GLOB: GLOB, PREFIX: PREFIX, title: S.title, slug: 'consent-gate-html-lint', price: 29 }); } catch (e) {}
}
