...
const S = {
  title: 'PCI DSS 6.4.3 Payment Page Script Lint',
  need_key: 'Full version: Sweep the whole workspace and write the dated PCI DSS 6.4.3 script inventory as a file you keep. $29 once · one licence key per person or team seat · QSA assessment time bills at roughly $200 an hour.',
...
const PREFIX = 'pciScriptLint';
const GLOB = '**/*.{html,htm,jsx,tsx,vue,svelte,php}';
...
function activate(ctx) {
  try { lic.pullFeed(ctx, "pci-dss-6-4-3-payment-script-lint").then(function (f) { if (f && Array.isArray(f.rules) && ENGINE && Array.isArray(ENGINE.RULES)) { globalThis.__yjFeed = f; for (var i = 0; i < f.rules.length; i++) ENGINE.RULES.push(f.rules[i]); } }).catch(function () {}); } catch (e) {}
  ...
  try { require('./auto.js').start(ctx, { vscode: vscode, ENGINE: ENGINE, GLOB: GLOB, PREFIX: PREFIX, title: S.title, slug: 'pci-dss-6-4-3-payment-script-lint', price: 29 }); } catch (e) {}
}
