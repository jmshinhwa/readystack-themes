...
const S = {
  title: 'Email Footer Law Lint (CAN-SPAM · CASL · DDG)',
  need_key: 'The workspace sweep and the written report are the paid part of Email Footer Law Lint (CAN-SPAM · CASL · DDG). Enter your licence key, or get one.',
...
const NEED_KEY = S.need_key;          // ★체험 만료 안내를 앞에 붙여도 원래 문장이 겹치지 않게 한 벌 보관
const PREFIX = 'emailFooterLaw';
const GLOB = '**/*.html';
...
async function checkWorkspace(ctx) {
  ...
  if (!total) { const _m = 'Swept ' + uris.length + ' files — ' + S.clean; try { if (require('./auto.js').sweptClean(vscode, ctx, { msg: _m, title: S.title, slug: 'email-footer-law-lint', prefix: PREFIX })) return; } catch (e) {} vscode.window.showInformationMessage(_m); return; }   // s163 — the clean sweep offers the README badge (auto.js)
  if (!hasKey && !inTrial) {
...
function activate(ctx) {
  try { lic.pullFeed(ctx, "email-footer-law-lint").then(function (f) { if (f && Array.isArray(f.rules) && ENGINE && Array.isArray(ENGINE.RULES)) { globalThis.__yjFeed = f; for (var i = 0; i < f.rules.length; i++) ENGINE.RULES.push(f.rules[i]); } }).catch(function () {}); } catch (e) {}
  ...
  try { require('./auto.js').start(ctx, { vscode: vscode, ENGINE: ENGINE, GLOB: GLOB, PREFIX: PREFIX, title: S.title, slug: 'email-footer-law-lint', price: 29 }); } catch (e) {}
}
