...
// ★역방향 체험 — ★첫 스윕부터 7일간 ★키 없이 ★전체 스윕과 보고서를 ⛔줄이지 않고 그대로 준다.
//   손님이 돈 낼지 정하는 순간은 ★자기 폴더에서 결과를 본 뒤다 ⇒ 유료 결과를 ★먼저 겪게 한다.
...
// ★체험 문 — 체험 중이면 ⛔키를 묻지 않는다. 끝나면 ★지난 스윕 숫자를 보여주고 키를 묻는다.
async function trialGate(ctx) {
  ...
  if (!inTrial) {
    ...
    S.need_key = (last && last.files ? ('Your trial sweep covered ' + last.files
      + ' files and found ' + last.findings + ' findings. ') : '') + NEED_KEY;
...
async function scanWorkspace(ctx) {
  ...
  const n = report(rows);
  // ★스윕한 숫자를 남긴다 — 체험이 끝난 뒤 ★손님이 본 것으로 키를 묻기 위해
  await t.st.update('lastSweep', { files: rows.length, findings: n,
