...
const S = {
  title: 'PCI Payment Page Script Audit',
  need_key: 'The workspace sweep and the written report are the paid part of PCI Payment Page Script Audit. Enter your licence key, or get one.',
...
const PREFIX = 'pciPaymentPageAudit';
const GLOB = '**/*.html';
...
function activate(ctx) {
  try { lic.pullFeed(ctx, "pci-payment-page-audit").then(function (f) { if (f && Array.isArray(f.rules) && ENGINE && Array.isArray(ENGINE.RULES)) { globalThis.__yjFeed = f; for (var i = 0; i < f.rules.length; i++) ENGINE.RULES.push(f.rules[i]); } }).catch(function () {}); } catch (e) {}
  ...
  try { require('./auto.js').start(ctx, { vscode: vscode, ENGINE: ENGINE, GLOB: GLOB, PREFIX: PREFIX, title: S.title, slug: 'pci-payment-page-audit', price: 29 }); } catch (e) {}
}
