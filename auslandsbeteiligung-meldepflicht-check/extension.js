...
const S = {
  title: 'Auslandsbeteiligung Meldepflicht-Check (§ 138 AO)',
  need_key: 'Vollversion: alle Register im Workspace prüfen und einen Bericht mit Frist, Rechtsgrund und Status für die Steuerakte schreiben. $39 einmalig, ein Lizenzschlüssel pro Person oder Team-Platz. Zum Vergleich: Steuerberater nach § 13 StBVV 16,50 € bis 41 € je angefangene Viertelstunde.',
  enter_key: 'Enter licence key', buy: 'Vollversion holen — $39',
...
const PREFIX = 'auslandsMeldung';
const GLOB = '**/*beteiligung*.csv';
...
async function checkWorkspace(ctx) {
  ...
  if (!total) { const _m = 'Swept ' + uris.length + ' files — ' + S.clean; try { if (require('./auto.js').sweptClean(vscode, ctx, { msg: _m, title: S.title, slug: 'auslandsbeteiligung-meldepflicht-check', prefix: PREFIX })) return; } catch (e) {} vscode.window.showInformationMessage(_m); return; }   // s163 — the clean sweep offers the README badge (auto.js)
  if (!hasKey && !inTrial) {
    S.need_key = 'This workspace: ' + total + ' finding' + (total === 1 ? '' : 's') + ' in ' + hit + ' of ' + uris.length + ' files. The full list and the written report are the paid part of ' + S.title + ' — $39 einmalig, ein Lizenzschlüssel pro Person oder Team-Platz. Steuerberater nach § 13 StBVV: 16,50 € bis 41 € je angefangene Viertelstunde.';
...
function activate(ctx) {
  ...
  try { lic.pullFeed(ctx, 'auslandsbeteiligung-meldepflicht-check').then(function (f) { if (f && Array.isArray(f.rules) && ENGINE && Array.isArray(ENGINE.RULES)) { globalThis.__yjFeed = f; for (var i = 0; i < f.rules.length; i++) ENGINE.RULES.push(f.rules[i]); } }).catch(function () {}); } catch (e) {}
  ...
  try { require('./auto.js').start(ctx, { vscode: vscode, ENGINE: ENGINE, GLOB: GLOB, PREFIX: PREFIX, title: S.title, slug: 'auslandsbeteiligung-meldepflicht-check', price: 29 }); } catch (e) {}
}
