...
const S = {
  title: 'targetSdk 36 Deadline Gate (Google Play)',
  need_key: 'The workspace sweep and the written report are the paid part of targetSdk 36 Deadline Gate (Google Play). Enter your licence key, or get one.',
...
const PREFIX = 'targetSdk36';
const GLOB = '**/{AndroidManifest.xml,build.gradle,build.gradle.kts}';
...
function activate(ctx) {
  try { lic.pullFeed(ctx, "target-sdk-36-deadline-gate").then(function (f) { if (f && Array.isArray(f.rules) && ENGINE && Array.isArray(ENGINE.RULES)) { globalThis.__yjFeed = f; for (var i = 0; i < f.rules.length; i++) ENGINE.RULES.push(f.rules[i]); } }).catch(function () {}); } catch (e) {}
  ...
  try { require('./auto.js').start(ctx, { vscode: vscode, ENGINE: ENGINE, GLOB: GLOB, PREFIX: PREFIX, title: S.title, slug: 'target-sdk-36-deadline-gate', price: 29 }); } catch (e) {}
}
