...
const S = {
  title: 'Bulk Sender Lint - Gmail, Yahoo, Outlook',
  need_key: 'The workspace sweep and the written report are the paid part of Bulk Sender Lint - Gmail, Yahoo, Outlook. Enter your licence key, or get one.',
...
const PREFIX = 'bulkSender';
const GLOB = '**/*.{js,ts,mjs,cjs,py,rb,php,tf,yml,yaml,zone,eml,txt}';
...
function activate(ctx) {
  try { lic.pullFeed(ctx, "bulk-sender-lint").then(function (f) { if (f && Array.isArray(f.rules) && ENGINE && Array.isArray(ENGINE.RULES)) { globalThis.__yjFeed = f; for (var i = 0; i < f.rules.length; i++) ENGINE.RULES.push(f.rules[i]); } }).catch(function () {}); } catch (e) {}
  ...
  try { require('./auto.js').start(ctx, { vscode: vscode, ENGINE: ENGINE, GLOB: GLOB, PREFIX: PREFIX, title: S.title, slug: 'bulk-sender-lint', price: 29 }); } catch (e) {}
}
