...
const S = {
  ...
  need_key: 'The workspace sweep and the written report are the paid part of EAA Form Lint: WCAG 2.2 AA for HTML. Enter your licence key, or get one. $29 once - one licence key per person or team seat. A WCAG audit from an accessibility vendor starts around $2,500 for one signup flow.',
...
const NEED_KEY = S.need_key;   // ★체험이 끝난 뒤 앞에 붙이는 문장이 겹쳐 쌓이지 않도록 원문을 잡아둔다
...
async function checkWorkspace(ctx) {
  ...
  if (!total) { const _m = 'Swept ' + uris.length + ' files — ' + S.clean; try { if (require('./auto.js').sweptClean(vscode, ctx, { msg: _m, title: S.title, slug: 'eaa-form-lint-wcag22', prefix: PREFIX })) return; } catch (e) {} vscode.window.showInformationMessage(_m); return; }   // s163 — the clean sweep offers the README badge (auto.js)
  if (!hasKey && !inTrial) {
...
function activate(ctx) {
  try { lic.pullFeed(ctx, "eaa-form-lint-wcag22").then(function (f) { if (f && Array.isArray(f.rules) && ENGINE && Array.isArray(ENGINE.RULES)) { globalThis.__yjFeed = f; for (var i = 0; i < f.rules.length; i++) ENGINE.RULES.push(f.rules[i]); } }).catch(function () {}); } catch (e) {}
  ...
  try { require('./auto.js').start(ctx, { vscode: vscode, ENGINE: ENGINE, GLOB: GLOB, PREFIX: PREFIX, title: S.title, slug: 'eaa-form-lint-wcag22', price: 29 }); } catch (e) {}
}
