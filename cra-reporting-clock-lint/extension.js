...
const S = {
  title: 'CRA Reporting Clock Lint (EU 2024/2847)',
  need_key: 'The workspace sweep and the written report are the paid part of CRA Reporting Clock Lint (EU 2024/2847). Enter your licence key, or get one.',
...
const PREFIX = 'craClock';
const GLOB = '**/SECURITY*.md';
...
function activate(ctx) {
  try { lic.pullFeed(ctx, "cra-reporting-clock-lint").then(function (f) { if (f && Array.isArray(f.rules) && ENGINE && Array.isArray(ENGINE.RULES)) { globalThis.__yjFeed = f; for (var i = 0; i < f.rules.length; i++) ENGINE.RULES.push(f.rules[i]); } }).catch(function () {}); } catch (e) {}
  ...
  try { require('./auto.js').start(ctx, { vscode: vscode, ENGINE: ENGINE, GLOB: GLOB, PREFIX: PREFIX, title: S.title, slug: 'cra-reporting-clock-lint', price: 29 }); } catch (e) {}
}
