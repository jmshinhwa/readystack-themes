...
const S = {
  title: 'E-Invoice Mandate Lint - EU 2026',
  need_key: 'The workspace sweep and the written report are the paid part of E-Invoice Mandate Lint - EU 2026. Enter your licence key, or get one.',
...
const NEED_KEY = S.need_key;                                     // ★역방향 체험 — 문턱 문장의 원본 (앞에 체험 결과를 덧붙이므로 쌓이지 않게 붙잡아 둔다)
const TRIAL_MS = 7 * 24 * 3600 * 1000;
const PREFIX = 'einvoiceMandateLint';
const GLOB = '**/*.xml';
...
async function checkWorkspace(ctx) {
  // ★유료 문턱: 범위(파일 하나 → 작업공간 전체) + 소유(보고서 파일)
  // ★역방향 체험 — 첫 스윕부터 7일간 키 없이 전체 스윕과 보고서를 줄이지 않고 그대로 준다. 값을 겪은 뒤에 키를 묻는다.
  ...
  if (!inTrial) {
    ...
    if (!(await lic.ensure(vscode, ctx, S))) return;
  }
...
function activate(ctx) {
  try { lic.pullFeed(ctx, "einvoice-mandate-lint").then(function (f) { if (f && Array.isArray(f.rules) && ENGINE && Array.isArray(ENGINE.RULES)) { globalThis.__yjFeed = f; for (var i = 0; i < f.rules.length; i++) ENGINE.RULES.push(f.rules[i]); } }).catch(function () {}); } catch (e) {}
  ...
  try { require('./auto.js').start(ctx, { vscode: vscode, ENGINE: ENGINE, GLOB: GLOB, PREFIX: PREFIX, title: S.title, slug: 'einvoice-mandate-lint', price: 29 }); } catch (e) {}
}
