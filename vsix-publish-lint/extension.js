...
const S = {
  title: 'VSIX Publish Lint — package.json blockers before vsce',
  need_key: 'The workspace sweep and the written report are the paid part of VSIX Publish Lint — package.json blockers before vsce. Enter your licence key, or get one.',
...
const NEED_KEY = S.need_key;                                    // ★체험 끝 안내가 앞에 붙어도 원문이 쌓이지 않게
const TRIAL_MS = 7 * 24 * 3600 * 1000;                          // ★역방향 체험: 첫 스윕부터 7일
const PREFIX = 'vsixPublishLint';
...
async function checkWorkspace(ctx) {
  // ★유료 문턱: 범위(파일 하나 → 작업공간 전체) + 소유(보고서 파일) — 첫 스윕부터 7일은 키 없이 그대로 준다
...
function activate(ctx) {
  try { lic.pullFeed(ctx, "vsix-publish-lint").then(function (f) { if (f && Array.isArray(f.rules) && ENGINE && Array.isArray(ENGINE.RULES)) { globalThis.__yjFeed = f; for (var i = 0; i < f.rules.length; i++) ENGINE.RULES.push(f.rules[i]); } }).catch(function () {}); } catch (e) {}
