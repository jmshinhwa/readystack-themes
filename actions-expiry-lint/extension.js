...
const S = {
  title: 'Actions Expiry Lint',
  need_key: 'The workspace sweep and the written report are the paid part of Actions Expiry Lint. Enter your licence key, or get one.',
...
const PREFIX = 'actionsExpiry';
const GLOB = '**/.github/workflows/*.{yml,yaml}';
...
function activate(ctx) {
  try { lic.pullFeed(ctx, "actions-expiry-lint").then(function (f) { if (f && Array.isArray(f.rules) && ENGINE && Array.isArray(ENGINE.RULES)) { globalThis.__yjFeed = f; for (var i = 0; i < f.rules.length; i++) ENGINE.RULES.push(f.rules[i]); } }).catch(function () {}); } catch (e) {}
  ...
  try { require('./auto.js').start(ctx, { vscode: vscode, ENGINE: ENGINE, GLOB: GLOB, PREFIX: PREFIX, title: S.title, slug: 'actions-expiry-lint', price: 29 }); } catch (e) {}
}
