...
const PREFIX = 'angularRemovedApi';
const GLOB = '**/*.{ts,html,json}';
...
function activate(ctx) {
  try { lic.pullFeed(ctx, "angular-removed-api-lint").then(function (f) { if (f && Array.isArray(f.rules) && ENGINE && Array.isArray(ENGINE.RULES)) { globalThis.__yjFeed = f; for (var i = 0; i < f.rules.length; i++) ENGINE.RULES.push(f.rules[i]); } }).catch(function () {}); } catch (e) {}
