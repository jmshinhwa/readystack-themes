...
const S = {
  title: 'Arbeitslosengeld-Check für Aufhebungsverträge',
  need_key: 'Vollversion: Prüfprotokoll mit allen Rechenschritten als Datei exportieren und alle Vertragsentwürfe im Workspace auf einmal prüfen. $29 einmalig · ein Lizenzschlüssel pro Person oder Team-Platz. Zum Vergleich: anwaltliche Erstberatung für Verbraucher bis zu 190 € (§ 34 RVG). Lizenzschlüssel eingeben oder holen.',
  enter_key: 'Enter licence key', buy: 'Vollversion holen — $29',
...
async function checkWorkspace(ctx) {
  ...
  if (!total) { const _m = 'Swept ' + uris.length + ' files — ' + S.clean; try { if (require('./auto.js').sweptClean(vscode, ctx, { msg: _m, title: S.title, slug: 'aufhebungsvertrag-sperrzeit-check', prefix: PREFIX })) return; } catch (e) {} vscode.window.showInformationMessage(_m); return; }   // s163 — the clean sweep offers the README badge (auto.js)
  if (!hasKey && !inTrial) {
...
function activate(ctx) {
  ...
  try { lic.pullFeed(ctx, 'aufhebungsvertrag-sperrzeit-check').then(function (f) { if (f && Array.isArray(f.rules) && ENGINE && Array.isArray(ENGINE.RULES)) { globalThis.__yjFeed = f; for (var i = 0; i < f.rules.length; i++) ENGINE.RULES.push(f.rules[i]); } }).catch(function () {}); } catch (e) {}
  ...
  try { require('./auto.js').start(ctx, { vscode: vscode, ENGINE: ENGINE, GLOB: GLOB, PREFIX: PREFIX, title: S.title, slug: 'aufhebungsvertrag-sperrzeit-check', price: 29 }); } catch (e) {}
}
