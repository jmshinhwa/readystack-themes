...
const S = {
  title: 'Firebase Deploy Leak Audit',
  need_key: 'The workspace sweep and the written report are the paid part of Firebase Deploy Leak Audit. Enter your licence key, or get one.',
...
const PREFIX = 'firebaseDeployAudit';
const GLOB = '**/firebase*.json';
...
function activate(ctx) {
  ...
  try { lic.pullFeed(ctx, 'firebase-deploy-leak-audit').then(function (f) { if (f && Array.isArray(f.rules) && ENGINE && Array.isArray(ENGINE.RULES)) { globalThis.__yjFeed = f; for (var i = 0; i < f.rules.length; i++) ENGINE.RULES.push(f.rules[i]); } }).catch(function () {}); } catch (e) {}
  ...
  try { require('./auto.js').start(ctx, { vscode: vscode, ENGINE: ENGINE, GLOB: GLOB, PREFIX: PREFIX, title: S.title, slug: 'firebase-deploy-leak-audit', price: 29 }); } catch (e) {}
}
