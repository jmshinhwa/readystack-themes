...
const S = {
  title: 'Impressum & Shop-Pflichtangaben Lint (DE)',
  need_key: 'The workspace sweep and the written report are the paid part of Impressum & Shop-Pflichtangaben Lint (DE). Enter your licence key, or get one.',
...
const PREFIX = 'impressumLintDe';
const GLOB = '**/*.{html,htm,md,vue,jsx,tsx,twig,liquid,php}';
...
function activate(ctx) {
  try { lic.pullFeed(ctx, "impressum-lint-de").then(function (f) { if (f && Array.isArray(f.rules) && ENGINE && Array.isArray(ENGINE.RULES)) { globalThis.__yjFeed = f; for (var i = 0; i < f.rules.length; i++) ENGINE.RULES.push(f.rules[i]); } }).catch(function () {}); } catch (e) {}
  ...
  try { require('./auto.js').start(ctx, { vscode: vscode, ENGINE: ENGINE, GLOB: GLOB, PREFIX: PREFIX, title: S.title, slug: 'impressum-lint-de', price: 29 }); } catch (e) {}
}
