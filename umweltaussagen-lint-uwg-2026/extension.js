...
const S = {
  title: 'Green-Claims-Lint: Umweltaussagen nach UWG 2026',
  need_key: 'The workspace sweep and the written report are the paid part of Green-Claims-Lint: Umweltaussagen nach UWG 2026. Enter your licence key, or get one.',
...
const PREFIX = 'umweltLint';
const GLOB = '**/*.{md,html,json,txt}';
...
async function checkWorkspace(ctx) {
  ...
  if (!total) { const _m = 'Swept ' + uris.length + ' files — ' + S.clean; try { if (require('./auto.js').sweptClean(vscode, ctx, { msg: _m, title: S.title, slug: 'umweltaussagen-lint-uwg-2026', prefix: PREFIX })) return; } catch (e) {} vscode.window.showInformationMessage(_m); return; }   // s163 — the clean sweep offers the README badge (auto.js)
  if (!hasKey && !inTrial) {
...
function activate(ctx) {
  ...
  try { lic.pullFeed(ctx, 'umweltaussagen-lint-uwg-2026').then(function (f) { if (f && Array.isArray(f.rules) && ENGINE && Array.isArray(ENGINE.RULES)) { globalThis.__yjFeed = f; for (var i = 0; i < f.rules.length; i++) ENGINE.RULES.push(f.rules[i]); } }).catch(function () {}); } catch (e) {}
  ...
  try { require('./auto.js').start(ctx, { vscode: vscode, ENGINE: ENGINE, GLOB: GLOB, PREFIX: PREFIX, title: S.title, slug: 'umweltaussagen-lint-uwg-2026', price: 29 }); } catch (e) {}
}
