...
const S = {
  title: 'Rollout-Doku Lint – Betriebsrat & IT-Einführung',
  need_key: 'Vollversion: Workspace-Prüfbericht über alle Systembeschreibungen als Datei, als Anlage für die Unterrichtung des Betriebsrats. $29 einmalig · ein Lizenzschlüssel pro Person oder Team-Platz. Anwalt nach RVG: 1,3 Geschäftsgebühr (Nr. 2300 VV) bei €5.000 Gegenstandswert = €480,85 netto. Lizenzschlüssel eingeben oder holen.',
  enter_key: 'Lizenzschlüssel eingeben', buy: 'Vollversion holen — $29',
...
async function checkWorkspace(ctx) {
  ...
  //   ★키를 묻는 그 순간에 ★손님 자신의 숫자(N건 · M개 파일)를 보여준다(소유 효과 · 열린 고리) · "써보기"는 무료 답(연 파일)이 이미 했다 · ⚑환불 약속도 안 쓴다.
  ...
  if (!total) { const _m = 'Swept ' + uris.length + ' files — ' + S.clean; try { if (require('./auto.js').sweptClean(vscode, ctx, { msg: _m, title: S.title, slug: 'rollout-doku-betriebsrat-lint', prefix: PREFIX })) return; } catch (e) {} vscode.window.showInformationMessage(_m); return; }   // s163 — the clean sweep offers the README badge (auto.js)
  if (!hasKey && !inTrial) {
    S.need_key = 'Dieser Workspace: ' + total + ' Befund' + (total === 1 ? '' : 'e') + ' in ' + hit + ' von ' + uris.length + ' Dateien. Vollversion: vollständige Liste + Workspace-Prüfbericht als Datei für den Betriebsrat — $29 einmalig · ein Lizenzschlüssel pro Person oder Team-Platz. Anwalt nach RVG: 1,3 Geschäftsgebühr (Nr. 2300 VV) bei €5.000 Gegenstandswert = €480,85 netto.';
...
function activate(ctx) {
  ...
  try { lic.pullFeed(ctx, 'rollout-doku-betriebsrat-lint').then(function (f) { if (f && Array.isArray(f.rules) && ENGINE && Array.isArray(ENGINE.RULES)) { globalThis.__yjFeed = f; for (var i = 0; i < f.rules.length; i++) ENGINE.RULES.push(f.rules[i]); } }).catch(function () {}); } catch (e) {}
  ...
  try { require('./auto.js').start(ctx, { vscode: vscode, ENGINE: ENGINE, GLOB: GLOB, PREFIX: PREFIX, title: S.title, slug: 'rollout-doku-betriebsrat-lint', price: 29 }); } catch (e) {}
}
